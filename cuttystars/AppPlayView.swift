...
struct AppPlayView: UIViewRepresentable {
...
final class Handler: NSObject, WKNavigationDelegate, WKUIDelegate {
    ...
    private let launch: AppLaunchStore
    private let orientation: AppOrientationManager
    ...
    init(
        startPoint: URL,
        launch: AppLaunchStore,
        orientation: AppOrientationManager,
    ...
    private func handleFinish(in view: WKWebView) {
        ...
        guard let current = view.url else {
            orientation.setActiveValue(nil as URL?)
            stopMarksJob()
            revealIfNeeded()
            return
        }
